  ...
  return (
    <div className="group relative min-w-[170px] rounded-xl border border-blue-500/50 dark:border-blue-500/40 bg-white/95 dark:bg-slate-900/90 p-3.5 shadow-lg shadow-blue-500/10 dark:shadow-blue-950/40 backdrop-blur-md transition-all duration-200 hover:border-blue-400 hover:shadow-blue-500/20">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[10px] font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400/90">Nivel (Stock)</span>
      </div>

      <div className="font-medium text-slate-900 dark:text-slate-100 text-sm tracking-tight">{data.label || 'Población'}</div>
      {data.subtext && (
        <div className="mt-1 font-mono text-xs text-blue-600 dark:text-blue-300/80 font-medium">
          {data.subtext}
        </div>
      )}
...
export const FlowNode = memo(({ data }) => {
  ...
  const borderColor = isOutflow 
    ? 'border-rose-500/50 dark:border-rose-500/40 hover:border-rose-500' 
    : 'border-emerald-500/50 dark:border-emerald-500/40 hover:border-emerald-500';
  const shadowColor = isOutflow 
    ? 'shadow-rose-500/10 dark:shadow-rose-950/40 hover:shadow-rose-500/20' 
    : 'shadow-emerald-500/10 dark:shadow-emerald-950/40 hover:shadow-emerald-500/20';
  const badgeBg = isOutflow 
    ? 'bg-rose-500/15 dark:bg-rose-500/20 text-rose-600 dark:text-rose-300 ring-rose-500/30' 
    : 'bg-emerald-500/15 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-300 ring-emerald-500/30';
  ...
  return (
    <div className={`group relative min-w-[160px] rounded-xl border ${borderColor} bg-white/95 dark:bg-slate-900/90 p-3 shadow-lg ${shadowColor} backdrop-blur-md transition-all duration-200`}>
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className={`text-[10px] font-semibold uppercase tracking-wider ${isOutflow ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
          {isOutflow ? 'Flujo de Salida' : 'Flujo de Entrada'}
        </span>
      </div>

      <div className="font-medium text-slate-800 dark:text-slate-200 text-sm">{data.label}</div>
      {data.equation && (
        <div className="mt-1 font-mono text-[11px] text-slate-500 dark:text-slate-400 truncate max-w-[150px]" title={data.equation}>
          = {data.equation}
        </div>
      )}
    ...
    <div className="group relative min-w-[140px] rounded-full border border-purple-500/40 dark:border-purple-500/35 bg-white/95 dark:bg-slate-900/90 px-4 py-2.5 shadow-lg shadow-purple-500/10 dark:shadow-purple-950/30 backdrop-blur-md transition-all duration-200 hover:border-purple-400 hover:shadow-purple-500/20">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          <span className="text-xs font-medium text-slate-800 dark:text-slate-200 leading-tight">{data.label}</span>
          {data.value !== undefined && (
            <span className="font-mono text-[11px] text-purple-600 dark:text-purple-400/90 font-medium">
              = {data.value}
            </span>
          )}
        </div>
